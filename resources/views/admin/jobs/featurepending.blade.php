    ...
    <x-slot name="MainContent">
        <div class="container-fluid">
            ...
            <ul class="nav nav-pills mb-4 d-flex justify-content-center flex-wrap">
                ...
                <li class="nav-item me-2 mb-2">
                    <a class="nav-link " href="/admin/jobs/pendingjobs">Pending Jobs</a>
                </li>
                <li class="nav-item me-2 mb-2">
                    <a class="nav-link text-white" href="/admin/jobs/featuredpending"
                        ...
                        <table class="table table-hover mb-0">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th scope="col" class="py-3 px-4 text-secondary text-center">Feature Status</th>
                                ...
                                @forelse ($jobs as $job)
                                    <tr>
                                        <td class="py-3 px-4 align-middle">
                                            ...
                                            <h6 class="card-subtitle mb-1 text-primary small" style="color: #007bff !important;"> Company : {{ $job->company_name ?? 'N/A' }}</h6>
                                            <p class="text-muted small mb-0 mt-2">
                                                <i class="bi bi-geo-alt me-1"></i> {{ $job->city }}, {{ $job->state }}
                                                <br>
                                        ...
                                        <td class="py-3 px-4 align-middle text-center">
                                            <span class="badge {{ $job->view_status == 'pending' ? 'bg-warning text-dark' : 'bg-success' }} rounded-pill">
                                                {{ ucfirst($job->view_status) }}
                                            </span>
                                        </td>
                                        <td class="py-3 px-4 align-middle text-center">
                                            <span class="badge {{ $job->featured_status == 'no' ? 'bg-warning text-dark' : 'bg-success' }} rounded-pill">
                                                {{ ucfirst($job->featured_status) }}
                                            </span>
                                        </td>
                                        <td class="py-3 px-4 align-middle text-end">
                                            <!-- View Details Button -->
                                            <a href="/admin/job-posts/{{$job->job_id}}" class="btn btn-outline-secondary btn-sm rounded-md">
                                    ...
                                    <tr>
                                        <td colspan="3" class="text-center text-muted py-4">
                                            <div class="alert alert-info text-center shadow-sm rounded-lg py-4 mb-0">
                                                <i class="bi bi-info-circle me-2"></i> No job posts found matching the criteria.
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
